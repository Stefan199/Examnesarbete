...
uint8_t LSBy, MSBy;
int data, dummy, full, val = 49, x=1;
int check = 0;
double dps,rpm;
uint8_t rpm_array[250];
...
//Setup
void setup() {
  Serial.begin(115200); // Serial definition for the Tx0 and Rx0
  Serial1.begin(9600);  // Serial definition for the Tx1 and Rx1
...
void loop() {
    ...
    if (Serial1.available() > 2) {
      val = Serial1.read();       // Read the information from the phone
      Serial.print(val);
      Serial.println("*");
      delay(100);
    }
   ...
   digitalWrite(CS, HIGH);
     ...
     // Combine the MSB byte with the LSB byte to get the full 16 bit data
     full = (MSBy<<8)+LSBy;

      //The sensor is giving out the data in form of 2 compliment values.
      //to convert the 2 complement value into one complement did I use the following ekuation.
     if(MSBy>=0x80){
        full = -((full^0xFFFF)+1);
      }

      //Calculate Degre per second (dps)
      // The 125 represent the maximum range the gyrosensor was sett to measure
      //Because the sensor is giving in 2 complement value, do the most significant bit 
      //represent if the value is positive/negative. That means that the maximum value the sensor
      //can write out is 2^(15)= 32768.
      dps = full*(125.00/32768); //It is only 15 bit that is used to declare valuse, the last bit is used to indicate if it is possetive or negative.

      //Calculate Revolution per minute (rpm)
      rpm = dps*(1.00/6);
    if(val == 49){
      Serial.println(rpm);      
      Serial1.println(rpm);
      delay(4);               // This is to send 250 values in 1 second, but not using buffer (1/250 = 0.004s = 4ms).
    }

  else{
      //This comment is to get how many bytes that is available for writing in the serial buffer
      //and will not block the write operator. I got it to 255 bytes. 
      /*int write_buffer_size = Serial1.availableForWrite();
      Serial.print(write_buffer_size);
      Serial.println("**");*/
      
      rpm_array[x]=rpm;
      
      //This is to almost use the whole write buffer.
      if(x=250){
        Serial1.write(rpm_array,250); // Test has been done with a bigger array, but the bluetooth module is 
        Serial1.flush(); //Make sure that the buffer is empty after the transmission
        x=1;
        delay(1000);     // this is to send 250 values in one second, but using a big part of the buffer.
        }
        x++; 
    }
} 
...
void test(){
   digitalWrite(CS, LOW);
      SPI.transfer(0xA1);   //Checking for error bit/message (INTERNAL_STATUS)
      dummy = SPI.transfer(0xFF);
      check = SPI.transfer(0xFF);
   digitalWrite(CS, HIGH); 

   //If the value is 1 the ASIC is initalized
   if(check == 1){
      Serial.print("OK");
      Serial.println(" ");
      
    }
   //Else show what error message it gives
   else{
      Serial.print("NOT OK//");
